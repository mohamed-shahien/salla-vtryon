...
export function OnboardingWizard() {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="max-w-lg p-0 overflow-hidden border-0 rounded-[16px] bg-card shadow-2xl" showCloseButton={false}>
        {/* Progress Bar */}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="flex justify-center">
                <div className={`p-6 rounded-[20px] ${steps[step-1].bg} ${steps[step-1].color} shadow-inner`}>
                  {(() => {
        ...
        <div className="p-6 bg-muted/30 border-t border-border flex items-center justify-between gap-4">
           {step > 1 ? (
             <Button 
                ...
                className="rounded-[8px] font-bold text-xs h-11 px-6 hover:bg-muted text-muted-foreground"
             >
           ...
           {step < steps.length ? (
             <Button 
                onClick={() => setStep(s => s + 1)}
                className="rounded-[8px] font-black text-xs h-11 px-8 bg-primary text-primary-foreground shadow-lg shadow-primary/20 hover:scale-105 transition-transform"
             >
                التالي
