...
import { Lock, RefreshCw, Loader2, ShieldCheck } from 'lucide-react'
...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    if (!token) {
      setError('رابط غير صالح. يرجى طلب رابط جديد لإعادة تعيين كلمة المرور.')
    }
  }, [token])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'فشل إعادة تعيين كلمة المرور. قد يكون الرابط منتهي الصلاحية.')
    } finally {
      setIsLoading(false)
    }
  }
        ...
        <div className="w-full max-w-md space-y-8 animate-in fade-in zoom-in duration-500">
          <Card className="p-8 border-border/50 shadow-2xl rounded-3xl bg-card/50 backdrop-blur-xl text-center space-y-6">
            ...
            <div className="space-y-2">
              <h1 className="text-2xl font-black tracking-tight text-foreground">تمت إعادة التعيين!</h1>
              <p className="text-muted-foreground">
                تم تغيير كلمة المرور الخاصة بك بنجاح. سيتم توجيهك إلى لوحة التحكم الآن...
              </p>
            </div>
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-background p-4 sm:p-6" dir="rtl">
      <div className="w-full max-w-md space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div className="flex flex-col items-center space-y-4">
          <div className="w-16 h-16 bg-primary rounded-2xl flex items-center justify-center shadow-xl shadow-primary/20 rotate-6 transition-transform hover:rotate-0">
            <RefreshCw className="w-8 h-8 text-white" />
          </div>
          <div className="text-center space-y-2">
            <h1 className="text-3xl font-black tracking-tight text-foreground">إعادة تعيين كلمة المرور</h1>
