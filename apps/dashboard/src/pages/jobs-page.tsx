...
const JobCard = memo(({
  ...
  return (
    <motion.div variants={item}>
      <Card
        className="group border-border/40 shadow-xs hover:shadow-xl transition-all duration-500 bg-card/70 backdrop-blur-md rounded-lg overflow-hidden hover:border-primary/30 relative h-full flex flex-col"
      >
        <CardContent className="p-3 space-y-4 text-right flex-1 flex flex-col">
          <div className="flex justify-between items-start">
            <Badge variant="outline" className={cn("text-[8px] font-black py-0 px-2 rounded-lg border-0 shadow-xs", status.color)}>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-1.5 justify-end">
              <Tooltip>
                <TooltipTrigger asChild>
                  <span className="flex items-center gap-1.5 bg-muted/60 px-2 py-0.5 rounded-lg border border-border/10 text-[9px] font-bold cursor-help">
              ...
              {job.category && (
                <span className="font-black text-[9px] text-primary/80 bg-primary/20 px-2 py-0.5 rounded-lg border border-primary/10">
          ...
          <div className="flex items-center gap-3 pt-3 border-t border-border/10 justify-between">
            <AvatarGroup>
              <Avatar className="size-11 border-2 border-background">
                ...
                <AvatarFallback className="rounded-lg text-[8px] font-black">U</AvatarFallback>
              </Avatar>
              <Avatar className="size-11 border-2 border-background">
                <AvatarImage src={productImage} alt="Product" className="object-cover" />
                <AvatarFallback className="rounded-lg text-[8px] font-black">P</AvatarFallback>
              </Avatar>
              {job.result_image_url && (
                <Avatar className="size-11 border-2 border-background ring-1 ring-primary/20">
                  ...
                  <AvatarFallback className="rounded-lg text-[8px] font-black">R</AvatarFallback>
                </Avatar>
              )}
            </AvatarGroup>
            ...
            <Button
              ...
              className="size-9 rounded-lg text-primary bg-primary/5 hover:bg-primary hover:text-white transition-all duration-300 shadow-sm active:scale-90"
            >
      ...
      <div className="space-y-3 animate-in fade-in duration-700 pb-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            <Card key={i} className={cn("border-border/40 shadow-xs bg-card/50 backdrop-blur-md rounded-lg overflow-hidden relative group hover:border-primary/20 transition-all duration-300", stat.border && `hover:border-${stat.border}/20`)}>
              <CardContent className="p-3 flex items-center justify-between">
                ...
                <div className={cn("size-9 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform", stat.bg ? `bg-${stat.bg}/5 text-${stat.bg}` : "bg-primary/5 text-primary")}>
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-3 bg-card/40 backdrop-blur-md p-3 rounded-lg border border-border/40 shadow-xs">
          <Tabs value={statusFilter} onValueChange={(val) => setStatusFilter(val as any)} className="w-full lg:w-auto">
            <TabsList className="bg-muted/50 p-1 h-9 rounded-lg border border-border/20 w-full lg:w-auto">
              {["all", "completed", "processing", "failed"].map((key) => (
                <TabsTrigger key={key} value={key} className={cn(
                  "text-[10px] font-black px-4 h-7 rounded-lg transition-all",
          ...
          <div className="relative w-full lg:w-72 group">
            ...
            <Input
              ...
              className="ps-10 h-9 bg-background border-border/60 focus:ring-primary/20 rounded-lg font-bold text-[10px] shadow-sm transition-all text-right"
            />
          </div>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <Card key={i} className="border-border/40 shadow-sm rounded-lg overflow-hidden bg-card/40">
          ...
          <Card className="border-destructive/20 bg-destructive/5 rounded-lg p-10 text-center">
            ...
            <Button onClick={() => void loadJobs()} variant="outline" className="rounded-lg font-black border-destructive/20 hover:bg-destructive/10 text-destructive h-10 px-6 shadow-sm">
              إعادة المحاولة
            </Button>
          </Card>
        ) : filteredJobs.length === 0 ? (
          <Card className="border-dashed border-border/60 bg-muted/20 rounded-lg p-16 text-center opacity-70">
        ...
        <Dialog open={!!selectedJob} onOpenChange={(open) => !open && setSelectedJob(null)}>
          <DialogContent className="max-w-3xl border-border/40 shadow-2xl rounded-lg bg-card/95 backdrop-blur-2xl p-0 overflow-hidden z-50 overflow-y-auto max-h-[90vh]" dir="rtl">
            {selectedJob && (
              <div className="animate-in fade-in zoom-in-95 duration-300">
                <DialogHeader className="p-5 border-b border-border/10 bg-linear-to-br from-muted/30 to-card text-right">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center w-full gap-3">
                    <div className="space-y-1">
                      <Badge variant="outline" className={cn("text-[9px] font-black px-3 py-0.5 rounded-lg border-0 shadow-xs", getStatusInfo(selectedJob.status).color)}>
                ...
                <div className="p-5 grid grid-cols-1 md:grid-cols-3 gap-5">
                  {[
                    ...
                    <div key={idx} className="space-y-3 text-right">
                      ...
                      <div className="aspect-3/4 rounded-lg overflow-hidden border border-border shadow-xl bg-muted relative group">
                        {img.url ? (
                          <>
                ...
                <DialogFooter className="p-4 bg-muted/60 border-t border-border/10 flex flex-row gap-3">
                  <Button onClick={() => setSelectedJob(null)} className="rounded-lg font-black text-xs px-8 h-10 bg-primary shadow-lg shadow-primary/20 hover:scale-105 active:scale-95 transition-all">إغلاق</Button>
                  {selectedJob.result_image_url && (
                    <a href={selectedJob.result_image_url} target="_blank" rel="noreferrer" className="flex-1">
                      <Button variant="outline" className="w-full rounded-lg font-black text-xs h-10 border-border/40 hover:bg-white transition-all">تحميل النتيجة</Button>
                    </a>
                  )}
                </DialogFooter>
              </div>
